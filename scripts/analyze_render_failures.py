...
위 등급은 표본 384개를 보고 "정상 셀(-12~-20)과 무너지는 셀(-34~-57)이 완전히
분리되니 그 사이인 -35 를 경계로 잡는다"고 정한 것이었다.
**전수 18,432 클립은 그렇지 않다.**

  -70  401 | -65 148 | -60 150 | -55 177 | -50 219 | -45 236 | -40 296
  -35  450 | -30 1,156 | -25 3,672 | -20 5,636 | -15 5,238 | -10 642

-65 부근의 얕은 최소를 지나 -30 까지 **단조 증가하는 연속 꼬리**다. 골짜기가 없다.
게다가 활성 구간 검출기가 무음이라 판정한 1,157개와 소리가 있다고 판정한 클립의
peak 범위가 겹쳐서, 어떤 단일 peak 경계로도 검출기 판정을 재현할 수 없다.

그래서 **활성 구간 검출기를 정본으로 쓴다**(qc_reasons 의 near_silence).
그쪽은 조정 상수가 아니라 절대 기준이다 — 어떤 프레임 RMS 도 -60 dBFS 를 못 넘으면
소리가 없는 것이다(audio.detect_active_region). metrics 도 그 순서로 거른다.

아래 peak 등급은 QC 를 아직 안 돌린 경우의 대용이며, 히스토그램은 경계가
정당한지 확인하기 위해서가 아니라 **정당하지 않음을 계속 보이기 위해** 출력한다.
전수 결과: 검출기 기준 생성 실패 1,157 / 18,432 = 6.28 %.
