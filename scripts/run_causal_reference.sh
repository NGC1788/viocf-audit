...
if "leaky" in verdict:
    ...
    if measured and identical == 0:
        ...
        print("    위 오프셋별 표를 볼 것. 실패의 '모양'이 원인을 말해준다:")
        print("      특정 오프셋만 통째로 -> 주입이 측정 창 밖에 떨어졌다")
        print("      모든 오프셋에서 정확히 절반 -> 주법 부류 하나가 통째로 빠졌다")
        print("        (감쇠 주법은 음 시작 뒤의 CC1 변화에 반응하지 않는다)")
        print("      고르게 흩어져 있다 -> 그때가 진짜 검정력 부족이다")
print()
