...
# manifests/config_robustness/wc<값>.csv 로 쪼개져 있다.
#
...
VIOCF_RESULTS="${VIOCF_ROOT}/results/config_robustness"
...
echo "=============================================================="
echo "설정 강건성 분석  (워커 ${VIOCF_WORKERS}개)"
echo "=============================================================="

VIOCF_PARTS=()
for VIOCF_MANIFEST in "${VIOCF_ROOT}"/manifests/config_robustness/*.csv; do
...
if [[ "${#VIOCF_PARTS[@]}" -eq 0 ]]; then
  echo "manifest 를 찾지 못했다: manifests/config_robustness/*.csv"
  echo "먼저: bash scripts/run_config_robustness.sh"
  exit 2
fi

# w_cc 별 표를 하나로 합친다. analyze_config_robustness.py 가 w_cc 열로 다시 가른다.
echo
echo "표 합치기"
...
echo
python "${VIOCF_ROOT}/scripts/analyze_config_robustness.py"
