...
## ⚠ 결정적 한계: MERT 는 음량을 보지 못한다

실측(합성음, 5초, 코사인 유사도. 1.0 = 구별 못함):

    A4 vs A4 (반복)      1.0000   결정적이다 (정상)
    A4 vs E5             0.6053   음정 구별함
    A4 vs A4 밝게        0.9046   음색 구별함
    A4 vs A4 -12 dB      1.0000   ← **음량은 전혀 구별 못함**

Wav2Vec2FeatureExtractor 가 입력을 zero-mean/unit-variance 로 정규화하기 때문이다.
MERT 는 그렇게 정규화된 입력으로 학습됐으므로 정규화를 끄면 분포 밖 입력이 된다.

**따라서 MERT 로 다이내믹(음량) 누출을 교차확인할 수 없다.**
이걸 모르고 쓰면 "MERT 로 봐도 다이내믹 누출이 없다" 는 거짓 결론이 나온다 —
MERT 는 애초에 볼 수가 없다.

다행히 역할 분담이 오히려 깔끔해진다:
  - **음량(목표 축)**: 해석가능 RMS 특징이 담당. ±0.5 dB 로 검증돼 있다(개정 1 참조).
  - **음색·음정·아티큘레이션(누출 축)**: MERT 가 교차확인. 우리 특징이 놓쳤을
    위험이 가장 큰 곳이 바로 여기다.

결과 해석 시 `loud` 군 특징은 MERT 축에서 제외하고 읽을 것.
